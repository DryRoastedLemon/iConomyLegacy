...
public class iConomy extends Plugin {
     ...
     * Miscellaneous object for various functions that don't belong anywhere else
     */
    public static Listens Listens = new Listens();
	...
	loader.addCustomListener(Listens.iSettings);
	loader.addCustomListener(Listens.iBalance);
	loader.addCustomListener(Listens.iLogging);
	loader.addCustomListener(Listens.iTemplate);
	...
	Logging = new iProperty(main_directory + "logging.settings");

	// Templating
	MoneyTPL = new Template("money.tpl");

	// Logging
	Info = new JSON("pay.log", Logging.getBoolean("log-pay", false));
